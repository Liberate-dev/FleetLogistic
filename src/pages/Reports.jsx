...
export default function Reports() {
  const { suratJalan, dispatches, pods, checklists, lpjRecords, fleet, addNotification } = useFleetOps();
  ...
  // Professional PDF download using jsPDF + autoTable
  const downloadReportAsPDF = () => {
    ...
    const reportLabel = selectedReport?.label || 'Report';
    const reportCode = selectedReport?.id?.toUpperCase() || '';
    const genDate = new Date().toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
    const genTime = new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' });
    const generatedBy = (auditLogger && auditLogger.currentUser) || 'Alex Sterling';

    // Header
    doc.setFillColor(70, 99, 71); // primary #466347
    doc.rect(0, 0, pageWidth, 18, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.text('FLEET OPS LOGISTICS', margin, 8);
    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    doc.text('Document Management System — Laporan Operasional', margin, 13);

    doc.setTextColor(255, 255, 255);
    doc.setFontSize(9);
    doc.text(`F-RP-01 | ${genDate}`, pageWidth - margin, 8, { align: 'right' });
    doc.text(`Generated at ${genTime} by ${generatedBy}`, pageWidth - margin, 13, { align: 'right' });
    ...
    doc.text(`Kode Laporan: ${reportCode}  •  Dicetak oleh: ${generatedBy}  •  ${genDate} pukul ${genTime} WIB`, margin, y);
    y += 8;
    ...
    const addFooter = () => {
      ...
      doc.text(`Halaman ${doc.internal.getCurrentPageInfo().pageNumber}`, pageWidth - margin, footY, { align: 'right' });
    };
    ...
    switch (selectedType) {
      case 'cash_advance': {
        ...
        const totalDisbursed = totalUangJalan + totalDanaCadangan;

        // KPIs
        const kpiW = (contentWidth - 4) / 3;
        drawKPI('Total Uang Jalan', formatCurrency(totalUangJalan), margin, kpiW);
        drawKPI('Total Dana Cadangan', formatCurrency(totalDanaCadangan), margin + kpiW + 2, kpiW);
        drawKPI('Total Disbursed', formatCurrency(totalDisbursed), margin + (kpiW + 2) * 2, kpiW);
        y += 18;

        doc.setFontSize(9);
        doc.setTextColor(70, 99, 71);
        doc.text('Detail Rekap Uang Jalan', margin, y);
        y += 3;

        const tableBody = cashAdvanceData.map(d => [
          d.number,
          d.destination || '-',
          formatCurrency(d.uangJalan),
          d.danaCadangan > 0 ? formatCurrency(d.danaCadangan) : '-',
          d.status || '-'
        ]);
        ...
        autoTable(doc, {
          ...
          head: [['SJ Number', 'Destination', 'Uang Jalan', 'Dana Cadangan', 'Status']],
          body: tableBody,
          styles: { fontSize: 8, cellPadding: 2, textColor: [33,37,41] },
          headStyles: { fillColor: [70, 99, 71], textColor: 255, fontStyle: 'bold', fontSize: 8 },
      ...
      case 'distance': {
        ...
        const kpiW = (contentWidth - 4) / 3;
        drawKPI('Total Distance', `${totalDistance.toLocaleString('id-ID')} km`, margin, kpiW);
        drawKPI('Rata-rata per Trip', `${avgDistance.toLocaleString('id-ID')} km`, margin + kpiW + 2, kpiW);
        drawKPI('Trip Terpanjang', `${maxDistance.toLocaleString('id-ID')} km`, margin + (kpiW + 2) * 2, kpiW);
        y += 18;

        const tableBody = distanceData.map(cl => [
          cl.id || '-',
          cl.vehiclePlate || '-',
          cl.driverName || '-',
          cl.date ? new Date(cl.date).toLocaleDateString('id-ID') : '-',
          `${Number(cl.odometerAwal || 0).toLocaleString('id-ID')} km`,
          `${Number(cl.odometerAkhir || 0).toLocaleString('id-ID')} km`,
          `${(cl.distanceTraveled || 0).toLocaleString('id-ID')} km`
        ]);

        autoTable(doc, {
          startY: y,
          margin: { left: margin, right: margin },
          head: [['Checklist ID', 'Vehicle', 'Driver', 'Date', 'Odo Awal', 'Odo Akhir', 'Distance']],
          body: tableBody,
          styles: { fontSize: 7.5, cellPadding: 1.5, textColor: [33,37,41] },
          headStyles: { fillColor: [70, 99, 71], textColor: 255, fontStyle: 'bold', fontSize: 7.5 },
          alternateRowStyles: { fillColor: [248, 249, 250] },
      ...
      case 'checklist': {
        ...
        const kpiW = (contentWidth - 4) / 3;
        drawKPI('Total Findings', totalFindings, margin, kpiW);
        drawKPI('Tidak Layak', tidakLayak, margin + kpiW + 2, kpiW);
        drawKPI('Perlu Perhatian', perluPerhatian, margin + (kpiW + 2) * 2, kpiW);
        y += 18;

        const tableBody = checklistFindings.map(f => [
          f.checklistId || '-',
          f.vehiclePlate || '-',
          f.driverName || '-',
          f.itemLabel || '-',
          f.status || '-',
          f.date ? new Date(f.date).toLocaleDateString('id-ID') : '-'
        ]);

        autoTable(doc, {
          startY: y,
          margin: { left: margin, right: margin },
          head: [['Checklist ID', 'Vehicle', 'Driver', 'Item', 'Status', 'Date']],
          body: tableBody,
          styles: { fontSize: 8, cellPadding: 2, textColor: [33,37,41] },
          headStyles: { fillColor: [70, 99, 71], textColor: 255, fontStyle: 'bold', fontSize: 8 },
          alternateRowStyles: { fillColor: [248, 249, 250] },
      ...
      case 'pod_disc': {
        const discrepancies = podDiscrepancies.length;
        const successRate = pods.length > 0 ? Math.round(((pods.length - discrepancies) / pods.length) * 100) : 100;

        const kpiW = (contentWidth - 4) / 3;
        drawKPI('Total PODs', pods.length, margin, kpiW);
        drawKPI('Discrepancies', discrepancies, margin + kpiW + 2, kpiW);
        drawKPI('Success Rate', `${successRate}%`, margin + (kpiW + 2) * 2, kpiW);
        y += 18;

        if (podDiscrepancies.length === 0) {
          doc.setFontSize(10);
          doc.setTextColor(40, 167, 69);
          doc.text('Tidak ada ketidaksesuaian. Semua pengiriman sukses.', margin, y);
        } else {
          const tableBody = podDiscrepancies.map(p => [
            p.number || '-',
            p.sjNumber || '-',
            p.receiver || '-',
            (p.condition || '').replace('_', ' '),
            (p.details || '-').substring(0, 60),
          ...
          autoTable(doc, {
            startY: y,
            margin: { left: margin, right: margin },
            head: [['POD Number', 'SJ Number', 'Received By', 'Condition', 'Details', 'Date']],
            body: tableBody,
            styles: { fontSize: 8, cellPadding: 2, textColor: [33,37,41] },
            headStyles: { fillColor: [186, 26, 26], textColor: 255, fontStyle: 'bold', fontSize: 8 },
      ...
      case 'client_perf': {
        ...
        const kpiW = (contentWidth - 4) / 3;
        drawKPI('Total Active Clients', totalClients, margin, kpiW);
        drawKPI('Total Deliveries', totalShipmentsAll, margin + kpiW + 2, kpiW);
        drawKPI('Avg Success Rate', `${averageSuccessRate}%`, margin + (kpiW + 2) * 2, kpiW);
        y += 18;

        const tableBody = clientPerformanceData.map(c => {
          const successRate = c.podsCount > 0 ? Math.round((c.goodPodsCount / c.podsCount) * 100) : 100;
          let statusText = successRate >= 95 ? 'Excellent' : successRate >= 80 ? 'Needs Attention' : 'Critical';
          return [
            c.name,
            c.totalShipments,
            c.completed,
        ...
        autoTable(doc, {
          startY: y,
          margin: { left: margin, right: margin },
          head: [['Client Name', 'Shipments', 'Completed', 'In Transit', 'PODs', 'Success %', 'Status']],
          body: tableBody,
          styles: { fontSize: 8, cellPadding: 2, textColor: [33,37,41] },
          headStyles: { fillColor: [69, 95, 135], textColor: 255, fontStyle: 'bold', fontSize: 8 },
          alternateRowStyles: { fillColor: [248, 249, 250] },
      ...
      case 'revenue': {
        ...
        const kpiW = (contentWidth - 4) / 3;
        drawKPI('Total Revenue', formatCurrency(totalRevenue), margin, kpiW);
        drawKPI('Total Tonnage', `${totalTonnage.toFixed(2)} Tons`, margin + kpiW + 2, kpiW);
        drawKPI('Avg Rev / Trip', formatCurrency(avgRevPerTrip), margin + (kpiW + 2) * 2, kpiW);
        y += 18;

        const tableBody = monthlyRevenueData.map(m => {
          const marginVal = m.revenue - m.lpjCost;
          return [
            m.label,
            m.trips,
            `${m.tonnage.toFixed(2)} T`,
            formatCurrency(m.revenue),
            m.lpjCost > 0 ? formatCurrency(m.lpjCost) : '-',
        ...
        autoTable(doc, {
          startY: y,
          margin: { left: margin, right: margin },
          head: [['Month', 'Trips', 'Tonnage', 'Revenue', 'Est. Expense (LPJ)', 'Est. Margin']],
          body: tableBody,
          styles: { fontSize: 8, cellPadding: 2, textColor: [33,37,41] },
          headStyles: { fillColor: [70, 99, 71], textColor: 255, fontStyle: 'bold', fontSize: 8 },
          alternateRowStyles: { fillColor: [248, 249, 250] },
      ...
      case 'fleet_util':
      default: {
        const totalAssets = (fleet.length || 3);
        const totalDisp = dispatches.length;
        const totalSJ = suratJalan.length;

        const kpiW = (contentWidth - 4) / 3;
        drawKPI('Total Assets', totalAssets, margin, kpiW);
        drawKPI('Total Dispatches', totalDisp, margin + kpiW + 2, kpiW);
        drawKPI('Total SJ', totalSJ, margin + (kpiW + 2) * 2, kpiW);
        y += 18;

        const assets = (fleet.length > 0 ? fleet : [
        ...
        const tableBody = assets.map(t => {
          const dispatchCount = dispatches.filter(d => d.truckId === t.id || d.truckPlate === t.plate).length;
          return [ `${t.id} (${t.plate || t.id})`, dispatchCount, t.status || 'READY' ];
        });

        autoTable(doc, {
          startY: y,
          margin: { left: margin, right: margin },
          head: [['Asset ID', 'Total Dispatches', 'Status']],
          body: tableBody,
          styles: { fontSize: 9, cellPadding: 2.5, textColor: [33,37,41] },
          headStyles: { fillColor: [70, 99, 71], textColor: 255, fontStyle: 'bold', fontSize: 9 },
          alternateRowStyles: { fillColor: [248, 249, 250] },
          didDrawPage: () => addFooter()
        });
        addFooter();
    ...
    // Filename safe
    const safeName = reportLabel.toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_|_$/g, '');
    const fName = `${safeName}_${new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(fName);
    } catch (err) {
      ...
      if (addNotification) {
        addNotification({
          ...
          message: 'Terjadi kesalahan saat membuat PDF. Coba gunakan tombol Print sebagai alternatif.'
        });
      } else {
        alert('Gagal membuat PDF report. Silakan coba Print.');
      }
    }
  };
    ...
    <Layout>
      <div className="flex-1 overflow-y-auto p-4 md:p-8 relative z-10 animate-fade-in no-scrollbar bg-slate-50/50 dark:bg-slate-900/50">
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="space-y-1 text-center mb-10 print:hidden">
          ...
          <div className="glass-panel p-8 rounded-3xl border border-white/60 shadow-xl space-y-8 print:hidden">
          ...
          {isGenerated && (
            <div className="mt-8 animate-fade-in">
              <div className="bg-white dark:bg-slate-800 rounded-t-lg border border-slate-200 border-b-0 p-4 flex justify-between items-center print:hidden">
                <button
                  onClick={() => window.print()}
                  className="flex items-center gap-1 text-slate-600 hover:text-primary text-sm font-bold"
                  title="Print using browser (saves as PDF too)"
                >
              ...
              <div id="report-content" className="bg-white dark:bg-slate-800 w-full shadow-2xl border border-slate-200 p-12 min-h-[600px] flex flex-col relative text-slate-800 print:shadow-none print:border-0 print:p-8 print:dark:bg-white print:text-slate-800">
                {renderReport()}
